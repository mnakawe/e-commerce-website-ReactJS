    ...
    return (
      <div className='container my-5'>
        <div className='empty-cart flex justify-center align-center flex-column font-manrope'>
          <img src = {shopping_cart} alt = "" style={{ width: '200px', height: '200px' }}/>
          <span className='fw-9 fs-24 text-dark'>Hey, it feels so light!</span>
          <span className='fw-5 fs-16 text-gray'>There is nothing in your bag. Let's add some items.</span>
          <Link to = "/" className='shopping-btn bg-orange text-pink fw-6'>Go shopping Now</Link>
        </div>
      </div>
    )
  }
              ...
              carts.map((cart, idx) => {
                return (
                  <div className='cart-ctr py-4' key = {cart?.id}>
                    ...
                    <div className='cart-ctd'>
                      <button type = "button" className='delete-btn text-dark' style={{textDecoration: 'underline'}} onClick={() => dispatch(removeFromCart(cart?.id))}>Delete</button>
                    </div>
                  </div>
                )
              })
